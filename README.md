...
The `type` attribute is the string to be compared with the `ro.aospa.build.variant` property.
The `version` attribute is displayed as the build version; the current build version is read
from `ro.aospa.version`.
